...
const Dashboard = () => {
  ...
  const processCallsData = (calls) => {
    ...
    setStatsData([
      { title: 'Calls Served', value: totalCalls, color: 'text-green-600' },
      { title: 'Redirected Calls', value: redirectedCalls, color: 'text-green-600' },
      { title: 'Bookings Made', value: bookingMadeCalls, color: 'text-green-600' },
    ...
    const routingData = [
      ...
      { name: 'LGBTQ+ Identity', value: nicheCounts['LGBTQ+ Identity'] || 0, color: '#14B8A6' },
    ...
    const providerPhoneNumbers = {
      ...
      'LGBTQ+ Identity': ['+18664887386', '+15052009086'],
  ...
  const fetchProvidersData = async () => {
    try {
      ...
      const providers = {
        ...
        'lgbtq-identity': [
          { name: 'LGBTQ+ Support Line', count: 1, time: '3:53 PM' }
        ],
        'elder-concern': [
  ...
  return (
    <div className="flex-1 flex flex-col overflow-x-hidden">
      ...
      {/* Content */}
      <main className="flex-1 px-4 md:px-8 py-6 md:py-8 bg-background overflow-y-auto overflow-x-hidden pb-32 md:pb-0">
        {/* Statistics Cards */}
              ...
              <div className="relative w-64 h-64">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                  {(() => {
                    ...
                    if (routingData.length === 1) {
                      ...
                      return (
                        <circle
                          ...
                          fill={seg.color}
                          onMouseEnter={() => {
                            setHoveredSegment({ ...seg, index: 0, percentage: '100.0' });
                    ...
                    // Multi-segment pie
                    return routingData.map((segment, index) => {
                ...
                {hoveredSegment && (
                  ...
                  >
                    ...
                    <div className="absolute left-0 top-1/2 transform -translate-x-1 -translate-y-1/2 w-0 h-0 border-t-4 border-b-4 border-r-4 border-transparent border-r-gray-200"></div>
                    <div className="absolute left-0 top-1/2 transform -translate-x-0.5 -translate-y-1/2 w-0 h-0 border-t-4 border-b-4 border-r-4 border-transparent border-r-white"></div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mt-4 space-y-2">
              {routingData.map((item, index) => {
                const categoryKey = item.name.toLowerCase()
                  ...
                  .replace('domestic-violence', 'domestic-violence')
                  .replace('youth-crisis', 'youth-crisis')
                  .replace('lgbtq+-identity', 'lgbtq-identity')
                  .replace('elder-care', 'elder-concern')
                  .replace('escalated-to-emergency', 'escalated-emergency');
              ...
              {(() => {
                const outcomesData = [
                  ...
                  { 
                    name: 'Escalated', 
                    value: callsData.filter(call => {
                      const n = getShortProviderName(call.provider_name, call).toLowerCase();
                      return n.includes('escalated') || n.includes('emergency');
                    }).length,
                    color: '#EF4444',
                    icon: '??'
                  }
                ...
                return (
                  <svg className="w-full h-full" viewBox="0 0 400 200">
                    ...
                    {outcomesData.map((outcome, index) => {
                      const barHeight = (outcome.value / 12) * 160;
                      const barWidth = 50;
