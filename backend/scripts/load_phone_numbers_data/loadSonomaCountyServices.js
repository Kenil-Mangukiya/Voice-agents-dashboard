...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/sonoma-county-services.json')
    ...
    // Delete existing Sonoma County records
    const deleteResult = await Service.deleteMany({ county: 'SONOMA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Sonoma County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SONOMA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Sonoma County services loaded successfully!`)
    } else {
