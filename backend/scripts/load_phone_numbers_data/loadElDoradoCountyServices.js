...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/el-dorado-county-services.json')
    ...
    // Delete existing El Dorado County records
    const deleteResult = await Service.deleteMany({ county: 'EL DORADO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing El Dorado County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'EL DORADO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ El Dorado County services loaded successfully!`)
    } else {
