...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/calaveras-county-services.json')
    ...
    // Delete existing Calaveras County records
    const deleteResult = await Service.deleteMany({ county: 'CALAVERAS', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Calaveras County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'CALAVERAS', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Calaveras County services loaded successfully!`)
    } else {
