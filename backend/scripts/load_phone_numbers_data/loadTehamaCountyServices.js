...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/tehama-county-services.json')
    ...
    // Delete existing Tehama County records
    const deleteResult = await Service.deleteMany({ county: 'TEHAMA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Tehama County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'TEHAMA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Tehama County services loaded successfully!`)
    } else {
