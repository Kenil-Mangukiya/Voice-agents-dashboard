...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/ventura-county-services.json')
    ...
    // Delete existing Ventura County records
    const deleteResult = await Service.deleteMany({ county: 'VENTURA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Ventura County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'VENTURA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Ventura County services loaded successfully!`)
    } else {
