...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/san-benito-county-services.json')
    ...
    // Delete existing San Benito County records
    const deleteResult = await Service.deleteMany({ county: 'SAN BENITO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing San Benito County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SAN BENITO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ San Benito County services loaded successfully!`)
    } else {
