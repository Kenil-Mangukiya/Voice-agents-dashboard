...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/imperial-county-services.json')
    ...
    // Delete existing Imperial County records
    const deleteResult = await Service.deleteMany({ county: 'IMPERIAL', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Imperial County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'IMPERIAL', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Imperial County services loaded successfully!`)
    } else {
