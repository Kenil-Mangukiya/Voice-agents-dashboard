...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/san-bernardino-county-services.json')
    ...
    // Delete existing San Bernardino County records
    const deleteResult = await Service.deleteMany({ county: 'SAN BERNARDINO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing San Bernardino County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SAN BERNARDINO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ San Bernardino County services loaded successfully!`)
    } else {
