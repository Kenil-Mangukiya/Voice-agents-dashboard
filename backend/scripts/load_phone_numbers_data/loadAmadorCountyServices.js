...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/amador-county-services.json')
    ...
    // Delete existing Amador County records
    const deleteResult = await Service.deleteMany({ county: 'AMADOR', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Amador County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'AMADOR', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Amador County services loaded successfully!`)
    } else {
