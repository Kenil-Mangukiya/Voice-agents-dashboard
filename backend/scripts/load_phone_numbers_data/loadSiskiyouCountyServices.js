...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/siskiyou-county-services.json')
    ...
    // Delete existing Siskiyou County records
    const deleteResult = await Service.deleteMany({ county: 'SISKIYOU', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Siskiyou County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SISKIYOU', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Siskiyou County services loaded successfully!`)
    } else {
