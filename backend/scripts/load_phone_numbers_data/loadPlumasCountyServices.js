...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/plumas-county-services.json')
    ...
    // Delete existing Plumas County records
    const deleteResult = await Service.deleteMany({ county: 'PLUMAS', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Plumas County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'PLUMAS', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Plumas County services loaded successfully!`)
    } else {
