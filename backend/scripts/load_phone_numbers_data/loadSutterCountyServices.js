...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/sutter-county-services.json')
    ...
    // Delete existing Sutter County records
    const deleteResult = await Service.deleteMany({ county: 'SUTTER', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Sutter County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SUTTER', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Sutter County services loaded successfully!`)
    } else {
