...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/madera-county-services.json')
    ...
    // Delete existing Madera County records
    const deleteResult = await Service.deleteMany({ county: 'MADERA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Madera County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'MADERA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Madera County services loaded successfully!`)
    } else {
