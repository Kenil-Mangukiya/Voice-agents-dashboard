...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/sacramento-county-services.json')
    ...
    // Delete existing Sacramento County records
    const deleteResult = await Service.deleteMany({ county: 'SACRAMENTO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Sacramento County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SACRAMENTO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Sacramento County services loaded successfully!`)
    } else {
