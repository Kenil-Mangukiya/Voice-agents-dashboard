...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/glenn-county-services.json')
    ...
    // Delete existing Glenn County records
    const deleteResult = await Service.deleteMany({ county: 'GLENN', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Glenn County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'GLENN', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Glenn County services loaded successfully!`)
    } else {
