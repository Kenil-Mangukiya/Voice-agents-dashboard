...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/inyo-county-services.json')
    ...
    // Delete existing Inyo County records
    const deleteResult = await Service.deleteMany({ county: 'INYO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Inyo County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'INYO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Inyo County services loaded successfully!`)
    } else {
