...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/colusa-county-services.json')
    ...
    // Delete existing Colusa County records
    const deleteResult = await Service.deleteMany({ county: 'COLUSA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Colusa County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'COLUSA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Colusa County services loaded successfully!`)
    } else {
