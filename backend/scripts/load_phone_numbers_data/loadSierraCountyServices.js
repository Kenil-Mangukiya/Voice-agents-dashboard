...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/sierra-county-services.json')
    ...
    // Delete existing Sierra County records
    const deleteResult = await Service.deleteMany({ county: 'SIERRA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Sierra County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SIERRA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Sierra County services loaded successfully!`)
    } else {
