...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/yolo-county-services.json')
    ...
    // Delete existing Yolo County records
    const deleteResult = await Service.deleteMany({ county: 'YOLO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Yolo County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'YOLO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Yolo County services loaded successfully!`)
    } else {
