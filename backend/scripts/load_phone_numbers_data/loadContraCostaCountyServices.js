...
/* =========================================================
   CONTRA COSTA COUNTY SERVICES LOADER
========================================================= */

async function loadContraCostaCountyServices() {
  try {
    console.log("\n");
    console.log("═".repeat(66));
    console.log("📍 CONTRA COSTA COUNTY - Services Loader");
    console.log("═".repeat(66));
    console.log();
    ...
    const jsonFilePath = path.join(
      __dirname,
      "../../db/phone_numbers_data/contra-costa-county-services.json"
    );
    ...
    console.log(`📄 Reading Contra Costa County services JSON...`);
    ...
    // Clear existing data for Contra Costa County
    console.log("🧹 Clearing existing CONTRA COSTA services...");
    ...
    if (validServices.length > 0) {
      ...
      const byPriority = await Service.aggregate([
        { $match: { county: "CONTRA COSTA", state: "CA" } },
      ...
      console.log(`Total CONTRA COSTA services in database: ${finalCount}\n`);
      ...
      console.log("   • County: 'CONTRA COSTA'");
    ...
    console.log("🎉 Contra Costa County services loaded successfully!\n");
    process.exit(0);
  } catch (error) {
    console.error("\n❌ Error loading Contra Costa County services:");
