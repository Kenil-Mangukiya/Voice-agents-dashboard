...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/tuolumne-county-services.json')
    ...
    // Delete existing Tuolumne County records
    const deleteResult = await Service.deleteMany({ county: 'TUOLUMNE', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Tuolumne County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'TUOLUMNE', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Tuolumne County services loaded successfully!`)
    } else {
