...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/lassen-county-services.json')
    ...
    // Delete existing Lassen County records
    const deleteResult = await Service.deleteMany({ county: 'LASSEN', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Lassen County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'LASSEN', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Lassen County services loaded successfully!`)
    } else {
