...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/kings-county-services.json')
    ...
    // Delete existing Kings County records
    const deleteResult = await Service.deleteMany({ county: 'KINGS', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Kings County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'KINGS', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Kings County services loaded successfully!`)
    } else {
