...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/san-joaquin-county-services.json')
    ...
    // Delete existing San Joaquin County records
    const deleteResult = await Service.deleteMany({ county: 'SAN JOAQUIN', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing San Joaquin County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SAN JOAQUIN', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ San Joaquin County services loaded successfully!`)
    } else {
