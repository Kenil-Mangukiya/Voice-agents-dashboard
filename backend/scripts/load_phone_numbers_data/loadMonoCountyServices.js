...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/mono-county-services.json')
    ...
    // Delete existing Mono County records
    const deleteResult = await Service.deleteMany({ county: 'MONO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Mono County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'MONO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Mono County services loaded successfully!`)
    } else {
