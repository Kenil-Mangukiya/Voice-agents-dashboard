...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/mendocino-county-services.json')
    ...
    // Delete existing Mendocino County records
    const deleteResult = await Service.deleteMany({ county: 'MENDOCINO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Mendocino County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'MENDOCINO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Mendocino County services loaded successfully!`)
    } else {
