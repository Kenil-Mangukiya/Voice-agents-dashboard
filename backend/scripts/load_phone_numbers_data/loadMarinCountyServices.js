...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/marin-county-services.json')
    ...
    // Delete existing Marin County records
    const deleteResult = await Service.deleteMany({ county: 'MARIN', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Marin County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'MARIN', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Marin County services loaded successfully!`)
    } else {
