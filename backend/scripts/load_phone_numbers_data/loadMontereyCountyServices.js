...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/monterey-county-services.json')
    ...
    // Delete existing Monterey County records
    const deleteResult = await Service.deleteMany({ county: 'MONTEREY', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Monterey County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'MONTEREY', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Monterey County services loaded successfully!`)
    } else {
