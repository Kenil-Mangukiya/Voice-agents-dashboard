...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/mariposa-county-services.json')
    ...
    // Delete existing Mariposa County records
    const deleteResult = await Service.deleteMany({ county: 'MARIPOSA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Mariposa County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'MARIPOSA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Mariposa County services loaded successfully!`)
    } else {
