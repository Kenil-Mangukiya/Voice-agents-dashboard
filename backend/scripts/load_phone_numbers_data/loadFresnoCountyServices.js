...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/fresno-county-services.json')
    ...
    // Delete existing Fresno County records
    const deleteResult = await Service.deleteMany({ county: 'FRESNO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Fresno County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'FRESNO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Fresno County services loaded successfully!`)
    } else {
