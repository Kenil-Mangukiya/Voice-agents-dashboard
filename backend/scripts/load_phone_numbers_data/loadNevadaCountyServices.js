...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/nevada-county-services.json')
    ...
    // Delete existing Nevada County records
    const deleteResult = await Service.deleteMany({ county: 'NEVADA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Nevada County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'NEVADA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Nevada County services loaded successfully!`)
    } else {
