...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/yuba-county-services.json')
    ...
    // Delete existing Yuba County records
    const deleteResult = await Service.deleteMany({ county: 'YUBA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Yuba County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'YUBA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Yuba County services loaded successfully!`)
    } else {
