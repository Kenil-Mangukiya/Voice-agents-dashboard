...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/riverside-county-services.json')
    ...
    // Delete existing Riverside County records
    const deleteResult = await Service.deleteMany({ county: 'RIVERSIDE', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Riverside County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'RIVERSIDE', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Riverside County services loaded successfully!`)
    } else {
