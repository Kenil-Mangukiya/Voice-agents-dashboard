...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/san-luis-obispo-county-services.json')
    ...
    // Delete existing San Luis Obispo County records
    const deleteResult = await Service.deleteMany({ county: 'SAN LUIS OBISPO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing San Luis Obispo County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SAN LUIS OBISPO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ San Luis Obispo County services loaded successfully!`)
    } else {
