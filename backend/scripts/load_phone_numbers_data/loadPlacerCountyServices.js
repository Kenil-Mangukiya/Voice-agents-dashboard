...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/placer-county-services.json')
    ...
    // Delete existing Placer County records
    const deleteResult = await Service.deleteMany({ county: 'PLACER', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Placer County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'PLACER', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Placer County services loaded successfully!`)
    } else {
