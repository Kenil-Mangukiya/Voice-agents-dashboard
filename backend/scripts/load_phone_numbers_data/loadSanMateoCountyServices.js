...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/san-mateo-county-services.json')
    ...
    // Delete existing San Mateo County records
    const deleteResult = await Service.deleteMany({ county: 'SAN MATEO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing San Mateo County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SAN MATEO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ San Mateo County services loaded successfully!`)
    } else {
