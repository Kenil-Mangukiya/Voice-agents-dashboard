...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/tulare-county-services.json')
    ...
    // Delete existing Tulare County records
    const deleteResult = await Service.deleteMany({ county: 'TULARE', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Tulare County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'TULARE', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Tulare County services loaded successfully!`)
    } else {
