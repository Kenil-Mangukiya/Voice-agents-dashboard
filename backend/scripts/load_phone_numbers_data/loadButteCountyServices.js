...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/butte-county-services.json')
    ...
    // Delete existing Butte County records
    const deleteResult = await Service.deleteMany({ county: 'BUTTE', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Butte County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'BUTTE', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Butte County services loaded successfully!`)
    } else {
