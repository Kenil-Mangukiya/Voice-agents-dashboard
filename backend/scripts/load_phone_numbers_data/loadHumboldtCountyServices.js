...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/humboldt-county-services.json')
    ...
    // Delete existing Humboldt County records
    const deleteResult = await Service.deleteMany({ county: 'HUMBOLDT', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Humboldt County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'HUMBOLDT', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Humboldt County services loaded successfully!`)
    } else {
