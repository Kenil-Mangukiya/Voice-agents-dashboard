...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/santa-barbara-county-services.json')
    ...
    // Delete existing Santa Barbara County records
    const deleteResult = await Service.deleteMany({ county: 'SANTA BARBARA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Santa Barbara County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SANTA BARBARA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Santa Barbara County services loaded successfully!`)
    } else {
