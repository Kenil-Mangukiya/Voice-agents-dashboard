...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/modoc-county-services.json')
    ...
    // Delete existing Modoc County records
    const deleteResult = await Service.deleteMany({ county: 'MODOC', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Modoc County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'MODOC', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Modoc County services loaded successfully!`)
    } else {
