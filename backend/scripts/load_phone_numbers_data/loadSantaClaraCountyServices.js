...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/santa-clara-county-services.json')
    ...
    // Delete existing Santa Clara County records
    const deleteResult = await Service.deleteMany({ county: 'SANTA CLARA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Santa Clara County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SANTA CLARA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Santa Clara County services loaded successfully!`)
    } else {
