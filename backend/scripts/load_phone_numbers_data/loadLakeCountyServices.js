...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/lake-county-services.json')
    ...
    // Delete existing Lake County records
    const deleteResult = await Service.deleteMany({ county: 'LAKE', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Lake County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'LAKE', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Lake County services loaded successfully!`)
    } else {
