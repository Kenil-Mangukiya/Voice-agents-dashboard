...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/stanislaus-county-services.json')
    ...
    // Delete existing Stanislaus County records
    const deleteResult = await Service.deleteMany({ county: 'STANISLAUS', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Stanislaus County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'STANISLAUS', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Stanislaus County services loaded successfully!`)
    } else {
