...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/napa-county-services.json')
    ...
    // Delete existing Napa County records
    const deleteResult = await Service.deleteMany({ county: 'NAPA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Napa County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'NAPA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Napa County services loaded successfully!`)
    } else {
