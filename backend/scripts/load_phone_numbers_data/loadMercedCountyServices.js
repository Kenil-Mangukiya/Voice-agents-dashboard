...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/merced-county-services.json')
    ...
    // Delete existing Merced County records
    const deleteResult = await Service.deleteMany({ county: 'MERCED', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Merced County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'MERCED', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Merced County services loaded successfully!`)
    } else {
