...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/san-diego-county-services.json')
    ...
    // Delete existing San Diego County records
    const deleteResult = await Service.deleteMany({ county: 'SAN DIEGO', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing San Diego County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SAN DIEGO', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ San Diego County services loaded successfully!`)
    } else {
