...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/alameda-county-services.json')
    ...
    // Delete existing Alameda County records
    const deleteResult = await Service.deleteMany({ county: 'ALAMEDA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Alameda County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'ALAMEDA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Alameda County services loaded successfully!`)
    } else {
