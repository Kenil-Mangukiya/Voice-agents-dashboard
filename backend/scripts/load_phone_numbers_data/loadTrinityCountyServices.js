...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/trinity-county-services.json')
    ...
    // Delete existing Trinity County records
    const deleteResult = await Service.deleteMany({ county: 'TRINITY', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Trinity County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'TRINITY', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Trinity County services loaded successfully!`)
    } else {
