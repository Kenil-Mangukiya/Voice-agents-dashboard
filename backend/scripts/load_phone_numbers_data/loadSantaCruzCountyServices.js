...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/santa-cruz-county-services.json')
    ...
    // Delete existing Santa Cruz County records
    const deleteResult = await Service.deleteMany({ county: 'SANTA CRUZ', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Santa Cruz County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SANTA CRUZ', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Santa Cruz County services loaded successfully!`)
    } else {
