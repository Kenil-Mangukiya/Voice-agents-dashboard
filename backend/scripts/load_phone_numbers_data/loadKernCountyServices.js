...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/kern-county-services.json')
    ...
    // Delete existing Kern County records
    const deleteResult = await Service.deleteMany({ county: 'KERN', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Kern County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'KERN', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Kern County services loaded successfully!`)
    } else {
