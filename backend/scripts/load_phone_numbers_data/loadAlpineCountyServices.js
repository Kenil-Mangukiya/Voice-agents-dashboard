...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/alpine-county-services.json')
    ...
    // Delete existing Alpine County records
    const deleteResult = await Service.deleteMany({ county: 'ALPINE', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Alpine County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'ALPINE', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Alpine County services loaded successfully!`)
    } else {
