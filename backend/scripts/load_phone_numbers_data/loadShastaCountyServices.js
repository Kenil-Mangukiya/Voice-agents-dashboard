...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/shasta-county-services.json')
    ...
    // Delete existing Shasta County records
    const deleteResult = await Service.deleteMany({ county: 'SHASTA', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Shasta County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'SHASTA', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Shasta County services loaded successfully!`)
    } else {
