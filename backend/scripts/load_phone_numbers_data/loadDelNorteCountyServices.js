...
async function loadCountyServices() {
  try {
    ...
    // Read JSON file
    const jsonPath = path.join(__dirname, '../../db/phone_numbers_data/del-norte-county-services.json')
    ...
    // Delete existing Del Norte County records
    const deleteResult = await Service.deleteMany({ county: 'DEL NORTE', state: 'CA' })
    console.log(`🗑️  Deleted ${deleteResult.deletedCount} existing Del Norte County records`)
    ...
    // Get final count
    const finalCount = await Service.countDocuments({ county: 'DEL NORTE', state: 'CA' })
    ...
    if (finalCount > 0) {
      console.log(`\n✅ Del Norte County services loaded successfully!`)
    } else {
